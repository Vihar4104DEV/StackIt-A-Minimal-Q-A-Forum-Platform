...
const Home = () => {
  ...
  useEffect(() => {
    const fetchPopularQuestions = async () => {
      try {
        setLoading(true);
        console.log('Fetching popular questions from API...');
        const response = await questionsApi.getQuestions({ 
          ordering: '-votes', 
          page_size: 4 
        });
        
        console.log('Popular questions response:', response);
        
        if (response && response.results) {
          setPopularQuestions(response.results);
        } else {
          console.log('No results in response, setting empty array');
          setPopularQuestions([]);
        }
      } catch (error) {
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-12">
            {loading ? (
              ...
              Array.from({ length: 4 }).map((_, index) => (
                <div key={index} className="question-card animate-pulse">
